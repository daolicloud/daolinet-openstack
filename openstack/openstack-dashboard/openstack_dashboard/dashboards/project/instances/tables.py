...
class TerminateInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def get_success_url(self, request):
        ...
        if not success_url:
            success_url = super(TerminateInstance, self).get_success_url(request)
        return success_url
...
class StartInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def get_success_url(self, request):
        success_url = request.POST.get('next')
        if not success_url:
            success_url = super(StartInstance, self).get_success_url(request)
        return success_url
...
class StopInstance(policy.PolicyTargetMixin, tables.BatchAction):
    ...
    def get_success_url(self, request):
        success_url = request.POST.get('next')
        if not success_url:
            success_url = super(StopInstance, self).get_success_url(request)
        return success_url
...
def get_size(instance):
    if hasattr(instance, "full_flavor"):
        return instance.full_flavor.name
...
def get_firewalls(instance):
    template_name = 'project/instances/_instance_firewall.html'
    context = {"id": instance.id, "name": instance.name}
    if hasattr(instance, "full_firewalls") and \
            len(instance.full_firewalls) > 0:
        context["firewalls"] = instance.full_firewalls
    return template.loader.render_to_string(template_name, context)
...
class InstancesTable(tables.DataTable):
    ...
    firewall_map = tables.Column(get_firewalls,
                                 verbose_name=_("Access Gateway (ssh)"))
    ...
    class Meta(object):
        ...
        row_actions = (StartInstance, EditInstance,
                       ConsoleLink, LogLink, TogglePause, ToggleSuspend,
                       SoftRebootInstance, RebootInstance,
                       StopInstance, TerminateInstance)
