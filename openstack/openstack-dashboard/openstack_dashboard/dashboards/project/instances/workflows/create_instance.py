...
class SetInstanceDetailsAction(workflows.Action):
    ...
    cidr = forms.ChoiceField(label=_("Subnet"),
                             help_text=_("Select a subnet."))
    ...
    def populate_cidr_choices(self, request, context):
        try:
            networks = api.daolicloud.network_list(request)
        except Exception:
            networks = []
        ...
        network_list.sort()
        if not network_list:
            network_list.insert(0, ("", _("No Network found")))
        return network_list
...
class SetInstanceDetails(workflows.Step):
    ...
    contributes = ("source_type", "source_id", "network_id",
...
class LaunchInstance(workflows.Workflow):
    ...
    def get_success_url(self):
        ...
        if not success_url:
            success_url = super(LaunchInstance, self).get_success_url()
        return success_url
    ...
    def handle(self, request, context):
        ...
        if not context.get('config_drive', None):
            context['config_drive'] = None
        ##############################
