...
(function (global, $) {
    ...
    gui = {
                ...
                if (nodes[i].status == "BUILD") {
                    ...
                    for (var j = 0; j < gui.share.nodes.length; j++) {
                        if (gui.share.nodes[j].id == nodes[i].id) {
                            ...
                            gui.share.nodes[j].firewalls = nodes[i].firewalls;
        ...
        update_status: function (image_get) {
          function update(idx) {
              $.ajax({
                url: prefix + "get/" + gui.share.nodes[idx].id,
                success: function (data) {
                  gui.share.nodes[idx] = data;
                  if (gui.share.nodes[idx].status == "BUILD") {
                    setTimeout(function() {update(idx)}, 1000);
                  } else if (gui.share.nodes[idx].status == "ERROR") {
                    _conf = config.status[gui.share.nodes[idx].status]
                    $("#vpc_details_" + data.id).find("image")
                      .attr("href", _conf.image).parent()
                      .find("text").text(_conf.text)
                      .data(data);
                  } else {
                    $("#vpc_details_" + data.id).find("image")
                      .attr("href", image_get(data.type)).parent()
                      .find("text").text(data.ip)
                      .data(data);
                  }
                },
                error: function(o) {
                  horizon.clearAllMessages();
                  horizon.alert("error", gettext('Failed to create the instance') + gui.share.nodes[i].name);
                }
              });
          }
          for (var i=0; i< gui.share.nodes.length; i++) {
        ...
        _events: {
            ...
            g_nodes: {
                mouseover: function(d) {
                  if (d.status != "ACTIVE" && d.status != "SHUTOFF") {
                    return;
                  }
                  ...
                  $(".ajax-modal, button[name=action]").click(function() {
                    $("#vpc_details_tip").popover('hide');
                  });
                },
                mouseout: function(d) {
                  $("#vpc_details_tip").siblings('.popover')
                    .mouseleave(function() {
                      $(this).popover('hide');
                  });
                },
