...
def instance_destroy(context, instance_uuid, constraint=None):
    ...
    with session.begin():
        ...
        # Delete other table.
        delete_by_instance(context, instance_uuid, session=session)
...
def network_get_all(context, project_only):
    # Modify project_only is allow_none.
...
def create_subnet(context, user_id, network_uuid, network=None, session=None):
    session = session or get_session()

    if network is None:
        network = network_get_by_uuid(context, network_uuid)

    net = netaddr.IPNetwork(network.cidr)
    gateway_ip = str(netaddr.IPAddress(net.last - 1))
...
def generate_ip(context, user_id, network_uuid):
    """Generate an IP address."""
    session = get_session()
    subnet = model_query(context, models.Subnet, session=session).\
            filter_by(user_id=user_id, network_id=network_uuid).first()
    if not subnet:
        create_subnet(context, user_id, network_uuid, session=session)
    ...
    range = query.filter_by(user_id=user_id, network_id=network_uuid).first()
...
def fixed_ip_add(context, instance_uuid, network_uuid, network_id, user_id,
    ...
    if address is None:
        address = generate_ip(context, user_id, network_uuid)
...
def fixed_ip_delete(context, address, instance_uuid):
    model_query(context, models.FixedIp).\
            filter_by(instance_uuid=instance_uuid).\
            filter_by(address=address).\
            delete()

def group_get_by_project(context, project_id):
    return model_query(context, models.Group).\
            options(joinedload_all('members')).\
            filter_by(project_id=project_id).all()

def group_get(context, id):
    return model_query(context, models.Group).\
            options(joinedload('members')).\
            filter_by(id=id).first()

def group_create(context, name, description, project_id):
    group_ref = models.Group()
    group_ref.name = name
    group_ref.description = description
    group_ref.project_id = project_id
    group_ref.save()
    return group_ref

def group_delete(context, id):
    session = get_session()
    ...
    with session.begin():
        ...
        if group:
            group_member_delete(context, group.id, session=session)

        query.delete()

def group_update(context, id, name=None, description=None):
    session = get_session()

    with session.begin():
        group = model_query(context, models.Group, session=session).\
                filter_by(id=id).first()
...
def group_member_get(context, group_id):
    session = get_session()
    return session.query(models.GroupMember).\
        filter_by(group_id=group_id).all()

def group_member_create(context, group_id, instance_id):
    session = get_session()
    member_ref = models.GroupMember()
    member_ref.group_id = group_id
    member_ref.instance_id = instance_id
    member_ref.save(session=session)
    return member_ref

def group_member_delete(context, group_id, instance_id=None, session=None):
    sessoin = session or get_session()
    query = session.query(models.GroupMember).\
        filter_by(group_id=group_id)
...
def gateway_get_all(context):
    return model_query(context, models.Gateway).all()
...
def firewall_create(context, **firewall):
    if not firewall.get('id'):
        firewall['id'] = uuidutils.generate_uuid()
...
def firewall_get(context, hostname=None, gateway_port=None):
    ...
    if not result:
        rule_str = '%s:%s' % (hostname, gateway_port)
        raise exception.SecurityGroupNotFoundForRule(rule_id=rule_str)

    return result

def delete_by_instance(context, instance_id, session=None):
    if session is None:
        session = get_session()
    with session.begin(subtransactions=True):
        model_query(context, models.Firewall).\
            filter_by(instance_id=instance_id).\
            delete()
        session.query(models.GroupMember).\
            filter_by(instance_id=instance_id).\
            delete()
        session.query(models.SinGroup).filter(or_(
            models.SinGroup.start==instance_id,
            models.SinGroup.end==instance_id)).\
            delete()

# Deprecated
def singroup_get_by_project(context, project_id):
    session = get_session()
    return sqlalchemyutils.model_query(models.SinGroup, session).\
        filter_by(project_id=project_id).all()

def singroup_create(context, project_id, start, end):
    session = get_session()
    group_ref = models.SinGroup()
...
def singroup_delete(context, project_id, start, end):
    session = get_session()

    with session.begin():
        query = sqlalchemyutils.model_query(models.SinGroup, session).\
            filter_by(project_id=project_id)

        query = query.filter(or_(
            and_(models.SinGroup.start==start, models.SinGroup.end==end),
            and_(models.SinGroup.start==end, models.SinGroup.end==start)))
