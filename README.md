...
There exists a number of offers for the Docker networking: Weave, Flannel, Libnetwork, and Colico are open source projects on Docker networking. These offers seemingly share a common requirement: Each Docker host participating a cloud must know network information of containers in other Docker hosts; e.g, Weave and Colico require that containers have external IP addresses which are visible by other Docker hosts; Flannel requires that Docker hosts run packet encapsulation, e.g., VXLAN, to tunnel inbetween different Doker hosts. However, as we have mentioned above, a Docker host is created without knowledge of other Doker hosts, let alone knowing containers in other Docker hosts. Requiring Docker hosts to know one another greatly complicates cloud management. In fact, the simplest and preferred network mode for Docker is Network Address Translation (NAT) in which containers in a Docker host do not have externally visible IP addresses. Under the NAT mode when a container communicates with the outside world, the Docker host uses its IP externally visible address to NAT translate the IP addresses of containers.
