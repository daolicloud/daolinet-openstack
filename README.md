...
---
Docker is a lightweight container engine and deserves an also lightweight networking solution for containers. From our descriptions on DaoliNet networking solution, we can see that an overlay network for distributed containers are pair-wise connected by the Controller to issue a pair of NAT-out, NAT-in flows. This pair of NAT flows are uniquely identified by src-PORT of a communications requestor. The Controller can maintain the uniqieness of src-PORT in the lifetime of a connection, and so this src-PORT number plays the role of a network isolation tag in the same manner of a packet encapsulation header tag does for isolating encapsulation tunnels, e.g., Virtual Network Identity (VNI) in VXLAN. Using no packet encapsulation, DaoliNet provides a lightweight network solution for connecting Docker containers. Obviously, the NAT-out and NAT-in defined "tunnel" does not encapsulate the overlay packets. Overlay network which is constructed from packet encapsulation, e.g., the way VXLAN (aka "network hypervisor") is used in Openstack Neutron, is too heavyweight for constructing overlay network for containers. 
