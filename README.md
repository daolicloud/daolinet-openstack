...
---
Docker is a lightweight container engine and therefore deserves a lightweight networking solution for containers, too. From our descriptions on DaoliNet networking solution, we can see that an overlay network for distributed containers are pair-wise connected by the Controller to packet-out a pair of NAT-out, NAT-in flows which are uniquely identified by src-PORT of a communications requestor. The Controller can maintain the uniqieness of src-PORT in the lifetime of a connection, and so src-PORT plays the role of a network isolation tag just as a packet encapsulation header does for isolating tunnels, e.g., the Virtual Network Identity (VNI) in VXLAN. Using no packet encapsulation, DaoliNet provides a lightweight network solution for connecting Docker containers. Obviously, the NAT-out and NAT-in defined "tunnel" does not encapsulate the overlay packets. Overlay network which is constructed from packet encapsulation, e.g., the way VXLAN (aka "network hypervisor") is used in Openstack Neutron, is too heavyweight for constructing overlay network for containers. 
