...
---
Docker is a lightweight container engine and therefore deserves a lightweight networking solution for containers, too. From our descriptions on DaoliNet networking solution, we can see that an overlay network for distributed containers are pair-wise connected by the Controller packet-out a pair of NAT-out, NAT-in flows which is uniquely identified by src-PORT of a communications requestor. The Controller can maintain the uniqieness of the src-PORT in the lifetime of a connection, and so src-PORT plays the role of a network tag in a packet encapsulation header for isolating tunnels, e.g., the Virtual Network Identity (VNI) in VXLAN. Isolation between This lightweight network virtualization technique uses no packet encapsulation for tunnel construction. Packet encapsulation, e.g., VXLAN (aka "network hypervisor") used in Openstack Neutron, is too heavyweight for constructing overlay network for containers. 
