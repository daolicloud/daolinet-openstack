...
commit_icmp_action(const struct flow *flow, struct flow *base_flow,
                             ...
                             struct flow_wildcards *wc)
{
    struct ovs_key_ipv4 ipv4_key, mask;
    memset(&ipv4_key, 0, sizeof(struct ovs_key_ipv4));
    memset(&mask, 0xff, sizeof(struct ovs_key_ipv4));
    if (base_flow->ipv6_label == flow->ipv6_label) {
        return;
    }
    if (is_ip_any(base_flow) && flow->nw_proto == IPPROTO_ICMP) {
        ipv4_key.ipv4_src = flow->ipv6_label;
        commit_masked_set_action(odp_actions, OVS_KEY_SET_ICMP_ID, &ipv4_key, &mask, sizeof(ipv4_key));
        base_flow->ipv6_label = flow->ipv6_label;
