...
static const int ovs_key_lens[OVS_KEY_ATTR_MAX + 1] = {
	...
	[OVS_KEY_ATTR_RECIRC_ID] = sizeof(u32),
        [OVS_KEY_SET_ICMP_ID] = sizeof(struct ovs_key_ipv4),//add by ljx
				...
				const struct nlattr **a, bool is_mask)
{
	...
	if (attrs & (1ULL << OVS_KEY_ATTR_ICMP)) {
		...
		SW_FLOW_KEY_PUT(match, tp.dst,
                ...
                //add by ljx
                SW_FLOW_KEY_PUT(match, ipv4.icmp.id,
                                icmp_key->icmp_identify, is_mask);				
		attrs &= ~(1ULL << OVS_KEY_ATTR_ICMP);
    
                //pr_info("######### ovs_key_from_nlattrs icmp_id: 0x%2x", match->key->ipv4.icmp.id);
	}
	...
	if ((swkey->eth.type == htons(ETH_P_IP) ||
		...
		if (swkey->ip.proto == IPPROTO_TCP) {
			...
			icmp_key->icmp_code = ntohs(output->tp.dst);
                        //add by ljx
                        icmp_key->icmp_identify = output->ipv4.icmp.id;		
		} else if (swkey->eth.type == htons(ETH_P_IPV6) &&
			...
			bool *set_tun)
{
	...
	int err;
	    ...
	    if (flow_key->ip.proto != IPPROTO_ICMP)
	        return -EINVAL;
	    if (flow_key->eth.type == htons(ETH_P_IP)){
	        if (flow_key->tp.src || flow_key->tp.dst || flow_key->ipv4.icmp.id){
                    break;
	        }
	    }
	    return -EINVAL;
