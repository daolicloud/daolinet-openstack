	...
	if (unlikely(err))
        ...
        if (ETH_P_ARP == ntohs(eth_hdr(skb)->h_proto)) {
            ...
            if(ARPOP_REQUEST == ntohs(p->ar_op)) {
    	        ...
    	        memcpy(pdata->ar_sha, eth_key->eth_src, ETH_ALEN);
...
static void parse_option(u8 *options, u8 opt, int *offset, __be32 addr, int type)
{
    options[(*offset)++] = opt;
    switch (opt)
    {
        case DHCP_SUBNETMASK:
            ...
            options[(*offset)+2] = 0x00;
            options[(*offset)+3] = 0x00;
            break;
        case DHCP_ROUTER:
        case DHCP_SERVIDENT:
            options[(*offset)++] = 0x04;
            options[*offset] = (u8)addr;
            options[(*offset)+1] = (u8)(addr >> 8);
            options[(*offset)+2] = (u8)(addr >> 16);
            options[(*offset)+3] = (u8)(addr >> 24);
            break;
        case DHCP_DNS:
            options[(*offset)++] = 0x04;
            options[*offset] = 0x08;
            options[(*offset)+1] = 0x08;
            options[(*offset)+2] = 0x08;
            options[(*offset)+3] = 0x08;
            break;
        case DHCP_LEASETIME:
            options[(*offset)++] = 0x04;
            options[*offset] = 0x00;
            options[(*offset)+1] = 0x01;
            options[(*offset)+2] = 0x51;
            options[(*offset)+3] = 0x80;
            break;
        case DHCP_MESSAGETYPE:
            options[(*offset)++] = 0x01;
            if (type == DHCPDISCOVER) {
                options[*offset] = DHCPOFFER;
            } else if(type == DHCPREQUEST) {
                options[*offset] = DHCPACK;
            }
            break;
    }
    if (opt != DHCP_END)
	...
	if (nh->protocol == IPPROTO_TCP) {
		...
		if (likely(transport_len >= sizeof(struct udphdr))) {
			...
			if (uh->source == htons(68) && uh->dest == htons(67)) {
				...
				struct dhcphdr *dhdr = (struct dhcphdr *)((u8 *)uh + sizeof(struct udphdr));
				u8 *options = (u8 *)dhdr + sizeof(struct dhcphdr);
				int type = options[2];
				dhdr->op = 0x02;
				uh->check = 0;
				if ((*addr >> 0x10) == 0xffff || (*addr >> 0x10) == 0xfeff) {
					dhdr->yiaddr.s_addr = new_addr;
					new_addr = *addr;
				} else {
					dhdr->siaddr.s_addr = new_addr;
					memset(options, 0, len);
					parse_option(options, DHCP_MESSAGETYPE, &ofset, new_addr, type);
					parse_option(options, DHCP_SERVIDENT, &ofset, new_addr, type);
					parse_option(options, DHCP_LEASETIME, &ofset, new_addr, type);
					parse_option(options, DHCP_SUBNETMASK, &ofset, new_addr, type);
					parse_option(options, DHCP_DNS, &ofset, new_addr, type);
					parse_option(options, DHCP_ROUTER, &ofset, new_addr, type);
					parse_option(options, DHCP_END, &ofset, new_addr, type);
				}
			}

			//if (uh->check || get_ip_summed(skb) == OVS_CSUM_PARTIAL) {
			if (uh->check || skb->ip_summed == CHECKSUM_PARTIAL) {
				inet_proto_csum_replace4(&uh->check, skb, *addr, new_addr, 1);
				if (!uh->check)
...
static int do_output(struct datapath *dp, struct sk_buff *skb, int out_port)
{
	...
	if (unlikely(!vport)) {
        ...
        //pr_info("######do_output:  call ovs_vport_send");
...
{
	int err = 0;
        pr_info("++++++++++++++execute_set_action+++++++++++++++++");
        pr_info("type: %d:", nla_type(nested_attr));
	...
	case OVS_KEY_ATTR_IPV4:
        ...
        err = set_icmp_identify(skb, nla_data(nested_attr));
        break;
	...
	for (a = attr, rem = len; rem > 0;
		...
		if (prev_port != -1) {
			...
			prev_port = -1;
                ...
                pr_info("+++++++++++++++do_execute_actions+++++++++++++++++++");
                pr_info("type: %d", nla_type(a));
		...
		case OVS_ACTION_ATTR_USERSPACE:
                        //pr_info("@@@@@@@@do_execute_actions: call output_userspace");
...
{ 
        ...
        ih = icmp_hdr(skb);
        if (*((__be16 *)&ipv4_key->ipv4_src) != ih->un.echo.id)
                //ih->un.echo.id = htons(*((__be16 *)&ipv4_key->ipv4_src));
                ih->un.echo.id = ipv4_key->ipv4_src;

        ih->checksum = 0;
        ih->checksum = ip_compute_csum((void *)ih, 
