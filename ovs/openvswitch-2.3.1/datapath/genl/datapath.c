...
#define IP_DEFRAG_OVS      (IP_DEFRAG_VS_FWD+ 1)
...
//                pr_info("ovs_ip_defrag: no fragment");
                return 0;
        }

//        pr_info("ovs_ip_defrag: call ip_defrag process fragmention");
        ...
        if (ip_defrag(skb, user)) {
                //pr_info("put skb in fragmention queue");
                return 1;
        }
...
/*
        ...
        case OVS_VPORT_TYPE_INTERNAL:
            ...
            //    pr_info("ovs_dp_process_received_packet: put skb in frag queue");
                return;
            }
            break;
        default:
...
*/
	...
	if (unlikely(!flow)) {
		struct dp_upcall_info upcall;
                /*u8 *src = (u8*)&key.ipv4.addr.src;
                u8 *dst = (u8*)&key.ipv4.addr.dst;

                if((*src == 192) && (*dst == 192)) 
                {
                    pr_info("OVS_PACKET_CMD_MISS: proto:%d, src:%d:%d:%d:%d  dst:%d:%d:%d:%d", 
                            key.ip.proto, *src, *(src+1), *(src+2),*(src+3),
			...
			bool *set_tun)
{
	...
	int err;
        case OVS_KEY_SET_ICMP_ID://add by ljx
                if (flow_key->ip.proto != IPPROTO_ICMP)
                        return -EINVAL;
		if (flow_key->eth.type == htons(ETH_P_IP)){
			if (flow_key->ipv4.tp.src || flow_key->ipv4.tp.dst || flow_key->ipv4.tp.id){
				break;
                        }
                }
                return -EINVAL;

        case OVS_KEY_SET_UDP: //add by changzhi
                //pr_info("ccccccccc validate set UDP xxxxxxxxx cccccccccc") ;
                break;

        case OVS_KEY_SET_EXT_UDP: //add by changzhi
                //pr_info("ccccccccc validate set UDP xxxxxxxxx cccccccccc") ;
                break;
	...
	{ &dp_packet_genl_family,
	  ...
	  NULL },
        { &doc_exmpl_gnl_family,
          doc_exmpl_gnl_ops_echo, ARRAY_SIZE(doc_exmpl_gnl_ops_echo),
          NULL },
};
