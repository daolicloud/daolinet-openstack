		    ...
		    int key_start, int key_end)
{
	...
	for (i = key_start; i < key_end;  i += sizeof(long)) {
                //pr_info("cp1 cp2: %016lx : %016lx", *cp1, *cp2);
		diffs |= *cp1++ ^ *cp2++;
        }

        if (diffs != 0) {
                pr_info("$$$$$$$$$$ start:%d end:%d", key_start, key_end);
		pr_info("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"); 
		x = (int *)key1;
		for (i = 0; i < sizeof(struct sw_flow_key)/4; i++) {
			pr_info("%08x", *x++);
		}
					  ...
					  u32 *n_mask_hit)
{
	...
	hlist_for_each_entry_rcu(flow, head, hash_node[ti->node_ver]) {
        ...
        //pr_info("-----------masked_flow_lookup: return null");
	return NULL;
}
				   ...
				   u32 *index)
{
	...
	if (*index < ma->max) {
		...
		if (mask) {
			...
			if (flow) {
                                //pr_info("flow_lookup: 111111111111111111");
				return flow;
                        }
		}
	}
	...
	for (i = 0; i < ma->max; i++)  {
		...
		if (flow) { /* Found */
			*index = i;
                        //pr_info("flow_lookup: 22222222222222222222"); 
			return flow;
		}
	}

        //pr_info("!!!!!!!!!!!!!!  flow_look return null");
