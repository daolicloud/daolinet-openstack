...
/*
static void print_miniflow(const struct miniflow *a)
{
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    /*
    VLOG_ERR("+++++++++++++add_flow+++++++++++++++++++++++++++++++++++++++++++++++++++");
    VLOG_ERR("fm->match.flow.ipv6_label:%x fm->match.wc.masks.ipv6_label:%x",
             fm->match.flow.ipv6_label, fm->match.wc.masks.ipv6_label); 
    VLOG_ERR("fm->match.flow.nw_src:%x fm->match.wc.masks.nw_src:%x",
             fm->match.flow.nw_src, fm->match.wc.masks.nw_src); 
    VLOG_ERR("fm->match.flow.nw_dst:%x fm->match.wc.masks.nw_dst:%x",
    ...
    */
    ...
    /*
    VLOG_ERR("++++++++add_flow-----cr.match.flow++++++++++++++++++++++++");
    print_miniflow(&cr.match.flow);
    VLOG_ERR("++++++++add_flow-----cr.match.mask++++++++++++++++++++++++");
    print_miniflow(&cr.match.mask.masks);
    */
    ...
    if (rule) {
        ...
        if (!rule_is_modifiable(rule, fm->flags)) {
            //VLOG_ERR("111111111111111111111111111111");
            return OFPERR_OFPBRC_EPERM;
        } else if (rule->pending) {
            //VLOG_ERR("22222222222222222222222222");
            return OFPROTO_POSTPONE;
        } else {
            struct rule_collection rules;
            ...
            rule_collection_destroy(&rules);
            //VLOG_ERR("33333333333333333333333");
            return error;
        }
    }
    ...
    if (is_flow_deletion_pending(ofproto, &cr, table_id)) {
        cls_rule_destroy(&cr);
        //VLOG_ERR("444444444444444444444444444444444");
        return OFPROTO_POSTPONE;
    }
    ...
    if (fm->flags & OFPUTIL_FF_CHECK_OVERLAP) {
        ...
        if (overlaps) {
            cls_rule_destroy(&cr);
            //VLOG_ERR("555555555555555555555555555");
    ...
    if (error) {
        cls_rule_destroy(&cr);
        //VLOG_ERR("666666666666666666666666");
        return error;
    }
    ...
    *CONST_CAST(uint8_t *, &rule->table_id) = table - ofproto->tables;
    ...
    if (error) {
        ofproto_rule_destroy__(rule);
        //VLOG_ERR("777777777777777777777777");
        return error;
    }
   
    /* 
    v = (int *)&rule->cr.match.flow;
    for (i=0; i < 16; i++) {
        VLOG_ERR("%08x", *v++);
    }
    VLOG_ERR("++++++++add_flow++++++++++++++++++++++++");
    */
    ...
    //VLOG_ERR("88888888888888888888888888888888888888888");
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    if (ofproto->n_pending < 50) {
        ...
        case OFPFC_ADD:
            ...
            //VLOG_ERR("------------add_flow returns: %x", error);
            break;
