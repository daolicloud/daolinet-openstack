               ...
               struct upcall *upcalls, size_t n_upcalls)
{
    ...
    HMAP_FOR_EACH (miss, hmap_node, misses) {
        ...
        */
        ...
        /*
        VLOG_ERR("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@+++++++++++++++++++++@@@@@@@@@@@@@@@@@@@@@@@@@@");
        VLOG_ERR("miss->flow.dl_type: %04x", miss->flow.dl_type);
        VLOG_ERR("miss->flow.nw_proto: %x", miss->flow.nw_proto);
        VLOG_ERR("miss->flow.nw_src: %08x", miss->flow.nw_src);
        VLOG_ERR("miss->flow.nw_dst: %08x", miss->flow.nw_dst);
        VLOG_ERR("miss->flow.ipv6_label: %04x", miss->flow.ipv6_label);
        VLOG_ERR("++++++++++++++call xlate_actions++++++++++++++++++++++++++++++++++++++++++++++");
        */
        ...
        /*
        VLOG_ERR("++++++++++++++exit xlate_actions++++++++++++++++++++++++++++++++++++++++++++++");
        pkey = (char *)miss->key;
        for (i = 0; i <= miss->key_len; i++) {
             sprintf(tmp + i, "%x", *pkey);
             pkey++;
        }
        VLOG_ERR("%s", tmp);
        VLOG_ERR("+++++++++++++++++++++++++++++");
    ...
    for (i = 0; i < n_upcalls; i++) {
        ...
        /*
        #ifndef NOTIFY_PATH
        #define NOTIFY_PATH "/var/ovs-agent"
        #endif
        struct hashkey {
        ...
        */
        ...
        if (may_put
            ...
            // daolicloud
            /*
            memcpy(hkey.packet, packet->data_, packet->size_);
            hkey.packet_len = packet->size_;
            if (miss->flow.in_port.ofp_port < USHRT_MAX - 1) {
                int i;
                hkey.odp_in_port = miss->odp_in_port;
                hkey.in_port = miss->flow.in_port.ofp_port;
                hkey.dp_ifindex = dpif_get_dp_ifindex((handler->udpif)->dpif);
                hkey.dl_type = miss->flow.dl_type;
                for (i=0; i<6; i++) {
                    hkey.dl_src[i] = miss->flow.dl_src[i];
                    hkey.dl_dst[i] = miss->flow.dl_dst[i];
                }
                hkey.nw_proto = miss->flow.nw_proto;
                hkey.nw_src = miss->flow.nw_src;
                hkey.nw_dst = miss->flow.nw_dst;
                hkey.tp_src = miss->flow.tp_src;
                hkey.tp_dst = miss->flow.tp_dst;
                if (miss->flow.nw_proto == 1) {
                    hkey.icmp_identify = htons(miss->flow.ipv6_label);
                } else {
                    hkey.icmp_identify = 0;
                }
                pfd = open(NOTIFY_PATH, O_WRONLY | O_NONBLOCK);
                if (pfd != -1) {
                    if (write(pfd, (char *)&hkey, sizeof(struct hashkey)) == -1) {
                        VLOG_ERR("write hashkey failed.");
                    }
                    close(pfd);
                }
            }
