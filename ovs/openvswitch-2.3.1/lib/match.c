...
//add by ljx
void
match_set_icmp(struct match *match, ovs_be16 id)
{
...
{
    //*(ovs_be16 *)((void *)&match->flow.ipv6_label) = port & mask;
    //*(ovs_be16 *)((void *)&match->wc.masks.ipv6_label) = mask;
    match_set_ipv6_label_masked(match, port, mask);
}
void
match_set_icmp_identify(struct match *match, ovs_be16 id)
{
    match_set_ipv6_label_masked(match, id, UINT16_MAX);
}
