                             ...
                             struct ofp10_match *ofmatch)
{
    ...
    //add bj ljx
    //FIX ME
    // *(ovs_be16 *)(ofmatch->pad2) = (*(ovs_be16*)(&match->flow.ipv6_label));
                        ...
                        ofp_port_t max_port, uint8_t max_table)
{
    ...
    if (raw == OFPRAW_OFPT11_FLOW_MOD) {
        ...
        /* 
        VLOG_ERR("--------------ofputil_decode_flow_mod"); 
        VLOG_ERR("icmp_id:mask %04x:%04x", 
        fm->match.flow.ipv6_label, fm->match.wc.masks.ipv6_label);
        VLOG_ERR("nw_src:mask %08x:%08x", 
        fm->match.flow.nw_src, fm->match.wc.masks.nw_src);
        VLOG_ERR("nw_dst:mask %08x:%08x", 
	...
	//VLOG_ERR("=========================================\n");
        ...
        if (raw == OFPRAW_OFPT10_FLOW_MOD) {
            ...
            /* Now get the actions. */
	    //VLOG_ERR("=======we now  into the openflow 10========\n");
	    ...
	    //VLOG_ERR("=======we now  into the openflow NXT_FLOW_MOD========\n");
            error = ofpacts_pull_openflow_actions(&b, ofpbuf_size(&b), oh->version,
                                                  ofpacts);
            if (error) {
                return error;
...
//   size_t num ;
	...
	//add by daolicloud
//tmp = ((char *)ofpbuf_data(reply) + start_ofs);
//num = ofpbuf_size(reply) - start_ofs ;
//memset(tmp + sizeof *ofs,0,num - sizeof *ofs);
	//end 
	...
	//add by daolicloud
	//memset(&ofs->match,0,sizeof ofs->match);
	//end
        ofs->duration_sec = htonl(fs->duration_sec);
        ofs->duration_nsec = htonl(fs->duration_nsec);
        ofs->priority = htons(fs->priority);
...
//tmp = ((char *)ofpbuf_data(reply) + start_ofs);
//num = ofpbuf_size(reply) - start_ofs ;
//memset(tmp + sizeof *nfs,0,num - sizeof *nfs);
	//end 
        ofpacts_put_openflow_actions(fs->ofpacts, fs->ofpacts_len, reply,
                                     version);
