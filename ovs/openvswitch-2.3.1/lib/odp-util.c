...
odp_flow_key_attr_len(uint16_t type)
{
    ...
    case OVS_KEY_SET_ICMP_ID: return sizeof(struct ovs_key_ipv4);
                    ...
                    bool verbose)
{
    ...
    case OVS_KEY_SET_ICMP_ID:
        //VLOG_ERR("-------------------format_odp_key_attr ----- fix me daolicloud-----------");
        break;
                  ...
                  const struct flow *src_flow)
{
    ...
    if (src_flow->nw_proto == IPPROTO_TCP
        ...
        if (present_attrs & (UINT64_C(1) << OVS_KEY_ATTR_ICMP)) {
            ...
            //VLOG_DBG("flow->ipv6_lable: 0x%2x", flow->ipv6_label); 
        }
...
//add by ljx
static void
commit_icmp_action(const struct flow *flow, struct flow *base,
                     struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
      struct ovs_key_ipv4 ipv4_key;
	  if( base->ipv6_label == flow->ipv6_label ){
	      return;
      }
      if (is_ip_any(base) && flow->nw_proto==IPPROTO_ICMP) {
	      memset(&wc->masks.ipv6_label, 0xff, sizeof wc->masks.ipv6_label);
             
              /*
              VLOG_ERR("++++++++++++++++++");
              VLOG_ERR("base->ipv6_label:%x flow->ipv6_lable:%x", base->ipv6_label,flow->ipv6_label);
              */
	      ipv4_key.ipv4_src = base->ipv6_label = flow->ipv6_label;

	      commit_set_action(odp_actions, OVS_KEY_SET_ICMP_ID,
		      &ipv4_key, sizeof(ipv4_key));
      }
}
