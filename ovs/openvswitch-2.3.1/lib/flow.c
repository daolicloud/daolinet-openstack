                 ...
                 struct miniflow *dst)
{
    ...
    //daolicloud
    struct mf_ctx mf_icmp_id = { 0, values, values + FLOW_U32S };
    char *l2;
    ovs_be16 dl_type;
    ...
    if (OVS_LIKELY(dl_type == htons(ETH_TYPE_IP))) {
        ...
        if (nh->ip_proto ==  IPPROTO_ICMP) {
            mf_icmp_id.data = mf.data;
            //placeholder for icmp id
            miniflow_push_be32(mf, ipv6_label, 0xFFFFFFFF);
        }
    ...
    if (OVS_LIKELY(!(nw_frag & FLOW_NW_FRAG_LATER))) {
        if (OVS_LIKELY(nw_proto == IPPROTO_TCP)) {
	        ...
	        //add by ljx
                //*(ovs_be16 *)(&flow->ipv6_label) = htons(icmp->icmp_fields.echo.id);
                //miniflow_push_be32(mf_icmp_id, ipv6_label, ntohs(icmp->icmp_fields.echo.id));
                miniflow_push_be32(mf_icmp_id, ipv6_label, icmp->icmp_fields.echo.id);
                mf.map |= mf_icmp_id.map;
            }
