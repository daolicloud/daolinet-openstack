                               ...
                               const struct flow *target)
{
    ...
    FLOW_FOR_EACH_IN_MAP(target_u32, target, mask->masks.map) {
        //VLOG_ERR("flowp:target_u32:maskp  %08x:%08x:%08x", *flowp, target_u32, *maskp);
        if ((*flowp++ ^ target_u32) & *maskp++) {
            err = false;
        }
    }
    /*
    if (!err)
        VLOG_ERR("+++++++++++miniflow_and_mask_matches_flow-----not match!!!!!!!!!!-------------");
    else
        VLOG_ERR("+++++++++++miniflow_and_mask_matches_flow-----match match match--------------------");
    */
